...
def verificarPalavraEhEntidade(palavra, entidades_loc):
	saida = ""
...
# Removendo stopwords.
# Removendo palavras que são entidades de Localização.
# Colocando todas as palavras para caixa baixa.
# Realizando lematização nas palavras.
# Adicionando apenas palavras: Substantivos, Adjetivos e Pronomes.
	...
	for token in doc:
		if (token.text not in gensim.parsing.preprocessing.STOPWORDS):
			if (token.pos_ in allowed_postags):
				...
				if (verificarPalavraEhEntidade(token, ents)):
...
model_list, coherence_values = compute_coherence_values(dictionary, corpus_tfidf, processed_docs, 15, 4, 1)
